...
/*   Updated: 2018/08/16 23:10:48 by amelihov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		paraboloid_fill_coeff(t_primitive primitive, t_basis basis,
				...
				t_ray ray, t_vect3d *intersect_point)
{
	...
	paraboloid_fill_coeff(primitive, basis, ray, coeff);
	if ((num_real_roots = solve2(coeff, roots)) == 0)
		return (0);
	if (!(num_pos_roots = sort_and_left_positive_roots(roots, num_real_roots)))
		return (0);
