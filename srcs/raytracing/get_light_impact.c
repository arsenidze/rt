...
/*   Updated: 2018/08/14 20:49:33 by amelihov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double		get_light_impact(const t_scene *scene, t_intersection *isect)
{
	...
	while (++i < (int)scene->lights.size)
	{
		...
		isect->shade = get_shade(scene->lights.data[i], isect, ray_to_light,
			sq_dist);
		impact += isect->shade * DIFFUSE;
		isect->glare = get_glare(scene->lights.data[i], isect, ray_to_light,
			sq_dist); 
		impact += isect->glare * SPECULAR;
	}
